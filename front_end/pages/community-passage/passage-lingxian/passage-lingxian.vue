		...
		<view class="image">
			<swiper indicator-dots="true" circular="true">
				<swiper-item v-for="(item, index) in contentImgList" :key="index">
					<image :src="item" mode="aspectFill" @click="previewpic(item,contentImgList)"></image>
				</swiper-item>
			</swiper>
		</view>
		...
		<view class="comment">
			...
			<view class="passage-comment" v-if="comments.length !== 0">
				...
				<view v-for="(item, index1) in comments" @click="openCommentsDetail(item)">
					...
					<view class="bottom">
						<view class="bottom_dianzan" @click.stop="likeComment(item)">
							<view class="ping-zan">
								<image :src="item.mainCommentBar.started?ping_star[1]:ping_star[0]"></image>
							</view>
							<!-- <uni-icons class="zan" type="hand-thumbsup" size="34" color="#888"></uni-icons> -->
	...
	export default {
		data() {
			return {
				...
				started: false,
				ping_started: false,
				ping_star: ["../../../static/relief/star.png", "../../../static/relief/star2.png"],
				userInfo: {},
			}
		},
		...
		methods: {
			backCommunity() {
				...
				const _this = this;
				uni.$emit('likeTab', {
					star: _this.articleDetail.star,
					started: _this.started,
					articleId: _this.articleDetail.article_id,
					index: _this.currentIndex,
				})
				uni.$emit('postComment', {
					commentNum: _this.articleDetail.comment,
					articleId: _this.articleDetail.article_id,
					index: _this.currentIndex,
				})
				uni.$emit('deleteArticle', {
					commentNum: _this.articleDetail.comment,
					articleId: _this.articleDetail.article_id,
					index: _this.currentIndex,
				})
			...
			likeComment(item) {
				...
				console.log(item);
				console.log(item.mainCommentBar.started);
				const _this = this;
				var is_star = item.mainCommentBar.started;
				uni.request({
					url: 'https://zk123.top/mye/c/star',
					method: 'POST',
					header: {
					...
					data: {
						id: item.mainCommentBar.commentId,
						flag: '1'
					},
					success: (res) => {
						...
						console.log(res);
						if (is_star == false) {
							item.mainCommentBar.started = true;
							console.log('now:' + item.mainCommentBar.started);
							item.mainCommentBar.star = item.mainCommentBar.star + 1;
							uni.showToast({
								title: '点赞成功'
							})
						} else {
							item.mainCommentBar.started = false;
							console.log('now:' + item.mainCommentBar.started);
							item.mainCommentBar.star = item.mainCommentBar.star - 1;
							uni.showToast({
								title: '取消点赞'
			...
			openCommentsDetail(item) {
				...
				uni.navigateTo({
					//穿参注意长度
					url: "../commentsDetail/commentsDetail?params=" + JSON.stringify(params)
				})
			},
			openPersonal(item) {
				console.log(item);
				const params = {
					userId: item.mainCommentBar.userId,
					nickname: item.mainCommentBar.nickname,
				}
				uni.navigateTo({
			...
			getArticleDetail(articleId) {
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.data.data.article.pic_url === null) {
							this.contentImgList = [];
						} else {
					...
					success: (res) => {
						...
						for (var i = 0; i < _this.comments.length; i++) {
							...
							uni.request({
								...
								success: (e) => {
									_this.comments[h].mainCommentBar.started = e.data.data;
									h++;
								},
							})
						...
						if (res.confirm) {
							uni.request({
								...
								success: (res) => {
									...
									console.log(item.mainCommentBar.articleId);
									_this.getArticleDetail(item.mainCommentBar.articleId);
								},
								fail: (err) => {
									console.log(err)
									return uni.showToast({
